...
# ---------------------------------------------------------------------------------
# Name: filters
# Description: When you filter a text, it auto responds to it if a user triggers the word)
# Author: HitaloSama
# Commands:
...
class FiltersMod(loader.Module):
    ...
    async def filtercmd(self, message):
        ...
        if not message.is_reply:
            if len(args) == 1:
                await message.edit(
                    "<b>Please reply to a message or enter a text to save as"
                    " filter.!</b>"
                )
                return
            else:
        ...
        await message.edit("<b>Successfully filtered.</b>".format(name))
        message.message = ""
    ...
    async def stopcmd(self, message):
        ...
        try:
            ...
            await message.edit(
                "<b>Filter </b><i>{}</i><b> successfully removed from the chat.</b>"
                .format(filtern)
            )
            self._db.set("FilterModule", "filters", filters)
        except KeyError:
            await message.edit(
                "<b>Filter </b><i>{}</i><b> not found in this chat</b>".format(filtern)
            )
        ...
        for key in filters[chatid]:
            if key in args:
                ...
                if not value.media and not value.web_preview:
                    ...
                    if exec is True:
                        argspr = arg.split(" ")
                        respond.message, cmd = self.allmodules.dispatch(
                            argspr[0], respond
                        )
                        await cmd(respond)
                else:
